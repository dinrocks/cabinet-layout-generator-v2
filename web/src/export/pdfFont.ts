 ...
 * standard Thai document font (Thai + Latin), register it on each jsPDF instance,
 * and the sheet SVG lists "Sarabun" first in font-family so svg2pdf resolves every
 * sheet text to it. PNG/preview fall back to Arial + the OS Thai fonts (Sarabun
 * isn't installed in the browser) — visually close; the PDF is the shipped one.
 */
import fontUrl from "../assets/Sarabun-Regular.ttf?url";

/** The face name registered with jsPDF and listed first in the sheet font-family. */
...
/** Register the Sarabun TTF (base64) on a jsPDF instance — split out so tests can
 *  feed the file straight from disk without the fetch. */
